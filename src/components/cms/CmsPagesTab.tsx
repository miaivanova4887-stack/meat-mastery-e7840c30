...
import { Globe, GlobeLock, Trash2, Layout, FileText, ExternalLink, Loader2 } from "lucide-react";
...
export default function CmsPagesTab({ onNavigateToLayout }: Props) {
  ...
  const allPages = [
    ...APP_PAGES.map(p => {
      ...
      return { title: p.title, slug: p.slug, path: p.path, isApp: true, layout, isPublished: layout?.is_published || false, updatedAt: layout?.updated_at || null };
    }),
    ...layouts.filter(l => !APP_PAGES.some(p => p.slug === l.page_slug)).map(l => ({
      title: l.title, slug: l.page_slug, path: `/p/${l.page_slug}`, isApp: false, layout: l, isPublished: l.is_published, updatedAt: l.updated_at,
    })),
  ];
  ...
  return (
    <div className="flex flex-col h-full">
      <div className="px-5 py-3 border-b border-border bg-card/50">
        ...
        <p className="text-[10px] text-muted-foreground">{allPages.length} pages — manage publish status and layouts</p>
      </div>
                ...
                <tr key={page.slug} className="hover:bg-accent/30 transition-colors">
                  <td className="py-2.5 pl-2">
                    <div className="flex items-center gap-1.5">
                      {page.isApp ? <Layout className="h-3.5 w-3.5 text-muted-foreground shrink-0" /> : <FileText className="h-3.5 w-3.5 text-muted-foreground shrink-0" />}
